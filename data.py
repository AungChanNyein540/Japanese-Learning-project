# -*- coding: utf-8 -*-
"""
Static learning content: kana tables and a JLPT N5 vocabulary set.
Kept as plain Python data structures so the Flask routes can filter,
sample, and quiz against them without a database.
"""

# Each row is a tuple of (kana, romaji). None marks an empty cell in the grid
# (e.g. "yi", "ye", "wu" don't exist in modern Japanese).
HIRAGANA_ROWS = [
    ["あ", "い", "う", "え", "お"],
    ["か", "き", "く", "け", "こ"],
    ["さ", "し", "す", "せ", "そ"],
...
KATAKANA_ROMAJI = HIRAGANA_ROMAJI  # identical romanization pattern
...
def paired_grid(rows, romaji_rows):
    """Zip a kana grid with its romaji grid into rows of {char, romaji} cells.

    Empty grid slots (None) stay None so templates can render a blank cell.
    Used by Django views, which keep this kind of prep out of the template.
    """
    grid = []
