...
def build_question(request, quiz_type: str):
    """Pick a random question and stash the correct answer in the session.

    Mirrors the Flask version: the client only ever receives the shuffled
    choice list, never a flag saying which one is correct.
    """
...
def hiragana(request):
    context = {
        "title": "Hiragana",
        "subtitle": "ひらがな",
        "description": "The rounded everyday script used for native Japanese words, "
...
def katakana(request):
    context = {
        "title": "Katakana",
        "subtitle": "カタカナ",
        "description": "The angular script used for foreign loanwords, "
